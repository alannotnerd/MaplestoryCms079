...
public class MapleCharacter extends AbstractAnimatedMapleMapObject implements InventoryContainer {
    ...
    while (rs.next()) {
      ...
      if ((type.equals(MapleInventoryType.EQUIP)) || (type.equals(MapleInventoryType.EQUIPPED))) {
        ...
        if (rs.getBoolean("isRing")) {
          ...
          equip.setItemExp(rs.getInt("itemexp"));//道具经验
          equip.setxingji(rs.getInt("xingji"));//星级
        }
        if (expiration != null) {
  ...
  public int getPetByUniqueId(int uniqueid) {
    for (int i = 0; i < this.pets.size(); i++) {
      if ((this.pets.get(i) != null) && (this.pets.get(i).getUniqueId() == uniqueid)) {
        return i;
      }
    }
  ...
  public void registerEffect(MapleStatEffect effect, long starttime, ScheduledFuture<?> schedule) {
    ...
    for (Pair<MapleBuffStat, Integer> statup : effect.getStatups()) {
      this.effects.put(statup.getLeft(), new MapleBuffStatValueHolder(effect, starttime, schedule, statup.getRight().intValue()));
    }
    recalcLocalStats();
  }
    ...
    for (MapleBuffStat stat : stats) {
      ...
      if (mbsvh != null) {
        ...
        if ((stat == MapleBuffStat.SUMMON) || (stat == MapleBuffStat.PUPPET)) {
          ...
          MapleSummon summon = this.summons.get(Integer.valueOf(summonId));
          if (summon != null) {
  ...
  public void cancelEffectFromBuffStat(MapleBuffStat stat) {
    if (this.effects.get(stat) != null) {
      cancelEffect(this.effects.get(stat).effect, false, -1L);
    }
  }
  ...
  public boolean hasEntered(String script, int mapId) {
    return (this.entered.containsKey(Integer.valueOf(mapId))) && (this.entered.get(Integer.valueOf(mapId)).equals(script));
  }
  ...
  public boolean hasEntered(String script) {
    for (Iterator i$ = this.entered.keySet().iterator(); i$.hasNext(); ) {
      ...
      if (this.entered.get(Integer.valueOf(mapId)).equals(script)) {
        return true;
      }
    }
    return false;
  ...
  public int getSkillLevel(int skill) {
    SkillEntry ret = this.skills.get(SkillFactory.getSkill(skill));
    if (ret == null) {
      return 0;
    }
  ...
  public void sendSpawnData(MapleClient client) {
    if (!isHidden() || (client.getPlayer().gmLevel() > 0)) {
...
///////////////////////////高级任务函数包//////////////////////////////////////
  ...
  public void cancelSavedBuffs() {
    ...
    for (Object o : keysarray) {
      ...
      cancelEffect(this.buffsToCancel.get(Long.valueOf(key)), false, key);
    }
