...
public class DebugCommands
  ...
  public void execute(MapleClient c, MessageCallback mc, String[] splitted)
    ...
    if (splitted[0].equals("!resetquest")) {
      ...
      List<MapleMapObject> reactors = map.getMapObjectsInRange(c.getPlayer().getPosition(), (1.0D / 0.0D), Arrays.asList(MapleMapObjectType.REACTOR));
      ...
      MapleMap map = c.getPlayer().getMap();
      List<MapleMapObject> items = map.getMapObjectsInRange(c.getPlayer().getPosition(), (1.0D / 0.0D), Arrays.asList(MapleMapObjectType.ITEM));
