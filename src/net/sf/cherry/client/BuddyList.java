...
public class BuddyList {
  ...
  public boolean containsVisible(int characterId) {
    BuddylistEntry ble = this.buddies.get(Integer.valueOf(characterId));
    if (ble == null) {
      return false;
    }
    ...
    try {
      ...
      while (rs.next()) {
        if (rs.getInt("pending") == 1) {
          this.pendingRequests.push(new CharacterNameAndId(rs.getInt("buddyid"), rs.getString("buddyname")));
          continue;
        }
