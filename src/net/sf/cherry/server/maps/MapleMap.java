...
public class MapleMap {
  ...
  public boolean isPQMap() { //不包括互动地图
    ...
    // 月兔pq
    //kpq + lpq only atm
    return (tmapid > 922010000 && tmapid < 922011100) ||
  ...
  public int playerCount() {
    List players = getMapObjectsInRange(new Point(0, 0), (1.0D / 0.0D), Arrays.asList(MapleMapObjectType.PLAYER));
    int count = players.size();
    return count;
  }

  public int mobCount() {
    return getMapObjectsInRange(new Point(0, 0), (1.0D / 0.0D), Arrays.asList(MapleMapObjectType.MONSTER)).size();
  }

  public void killAllmonster() {
    List<MapleMapObject> monsters = getMapObjectsInRange(new Point(0, 0), (1.0D / 0.0D), Arrays.asList(MapleMapObjectType.MONSTER));
    for (MapleMapObject monstermo : monsters) {
      MapleMonster monster = (MapleMonster) monstermo;
