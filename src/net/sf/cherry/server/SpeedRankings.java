...
public class SpeedRankings {
  ...
  public static long calcTime(int type, int oid) {
    ...
    try {
      if (type == 0) {
        time = zEndTime.get(Integer.valueOf(oid)).longValue() - zStartTime.get(Integer.valueOf(oid)).longValue();
      } else if (type == 1) {
        time = pEndTime.get(Integer.valueOf(oid)).longValue() - pStartTime.get(Integer.valueOf(oid)).longValue();
      }
    } catch (Exception e) {
      //e.printStackTrace();
    }
