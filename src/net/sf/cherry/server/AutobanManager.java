...
public class AutobanManager
  ...
  public synchronized void addPoints(MapleClient c, int points, long expiration, String reason) {
    ...
    if (this.points.containsKey(Integer.valueOf(acc))) {
      if (this.points.get(Integer.valueOf(acc)).intValue() >= 1000) {
        return;
      }
      this.points.put(Integer.valueOf(acc), Integer.valueOf(this.points.get(Integer.valueOf(acc)).intValue() + points));
      List reasonList = this.reasons.get(Integer.valueOf(acc));
  ...
  public void run() {
    ...
    for (ExpirationEntry e : this.expirations) {
      if (e.time <= now) {
        this.points.put(Integer.valueOf(e.acc), Integer.valueOf(this.points.get(Integer.valueOf(e.acc)).intValue() - e.points));
      } else {
        return;
      }
    }
