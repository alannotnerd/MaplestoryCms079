...
public class NPCConversationManager extends AbstractPlayerInteraction {
  ...
  public boolean PdMapPlayer(int mapid) {//判断地图是否有人
    ...
    //如果等于1人或者大于1人
    //房间不可用
    return !mf.getMap(mapid).getAllPlayers().isEmpty(); //房间状态显示
  }
  ...
  public boolean PdMapMob(int mapid, int mobid) {
    ...
    //如果等于1人或者大于1人
    //房间不可用
    return mf.getMap(mapid).getMonsterById(mobid) == null; //房间状态显示
  }
  //高级任务系统 - 放弃所有未完成任务
  ...
  public List<Integer> getMapMobsIds() {
    ...
    List<MapleMapObject> monsters = map.getMapObjectsInRange(this.c.getPlayer().getPosition(), range, Arrays.asList(MapleMapObjectType.MONSTER));
...
//  }
  ...
  public int countMonster() {
    MapleMap map = this.c.getPlayer().getMap();
    double range = (1.0D / 0.0D);
    List monsters = map.getMapObjectsInRange(this.c.getPlayer().getPosition(), range, Arrays.asList(MapleMapObjectType.MONSTER));
  ...
  public int countReactor() {
    MapleMap map = this.c.getPlayer().getMap();
    double range = (1.0D / 0.0D);
    List reactors = map.getMapObjectsInRange(this.c.getPlayer().getPosition(), range, Arrays.asList(MapleMapObjectType.REACTOR));
  ...
  public boolean fieldLobbied(int field) { //判断场地
    //如果等于1人或者大于1人
    //房间不可用
    return c.getChannelServer().getMapFactory().getMap(980000100 + field * 100).getAllPlayers().size() >= 1; //房间状态显示
  }

  public boolean fieldTaken(int field) { //战场  选择
    MapleMapFactory mf = c.getChannelServer().getMapFactory(); //的到地图工厂
    return (!mf.getMap(980000100 + field * 100).getAllPlayers().isEmpty()) || (mf.getMap(980000101 + field * 100).getAllPlayers().size() != 0) || (mf.getMap(980000102 + field * 100).getAllPlayers().size() != 0);
  }
