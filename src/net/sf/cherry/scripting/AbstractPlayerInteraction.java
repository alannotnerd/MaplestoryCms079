...
public class AbstractPlayerInteraction {
    ...
    if (quantity >= 0) {
      ...
      if ((type.equals(MapleInventoryType.EQUIP)) && (!ii.isThrowingStar(item.getItemId())) && (!ii.isBullet(item.getItemId()))) {
        if (randomStats) {
          ...
          MapleInventoryManipulator.addFromDrop(this.c, item, logInfo.toString(), false);
        }
      } else {
  ...
  public void resetMap(int mapid) {
    ...
    for (MapleMapObject i : getMap(mapid).getMapObjectsInRange(this.c.getPlayer().getPosition(), (1.0D / 0.0D), Arrays.asList(MapleMapObjectType.ITEM))) {
