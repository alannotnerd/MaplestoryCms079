...
public class ChannelServer
  ...
  public static ChannelServer getInstance(int channel) {
    ...
    try {
      ret = instances.get(Integer.valueOf(channel));
  ...
  public MapleGuildSummary getGuildSummary(int gid) {
    if (this.gsStore.containsKey(Integer.valueOf(gid))) {
      return this.gsStore.get(Integer.valueOf(gid));
    }
    try {
    ...
    public void run() {
      ...
      for (int x = allplayers.size() - 1; x >= 0; x--) {
        ...
        if (cheater.getCheatTracker().getPoints() > 0) {
          if ((cheater.getCheatTracker().getPoints() <= 5000) || ((cheater.getCheatTracker().getOffenses().get(CheatingOffense.MOVE_MONSTERS) != null) && (cheater.getCheatTracker().getOffenses().get(CheatingOffense.MOVE_MONSTERS).getCount() > 1000) && ((cheater.getMapId() < 230000000) || (cheater.getMapId() > 240000000)))) {
            cheater.doCherryAutoban("MOVE_MONSTERS");
          }
          if ((cheater.getCheatTracker().getOffenses().get(CheatingOffense.ATTACK_WITHOUT_GETTING_HIT) != null) && (cheater.getCheatTracker().getOffenses().get(CheatingOffense.ATTACK_WITHOUT_GETTING_HIT).getCount() > 1000) && (cheater.getJob().getId() / 10 == 23)) {
            cheater.doCherryAutoban("ATTACK_WITHOUT_GETTING_HIT");
          }
          if ((cheater.getCheatTracker().getOffenses().get(CheatingOffense.FASTATTACK) != null) && (cheater.getCheatTracker().getOffenses().get(CheatingOffense.FASTATTACK).getCount() > 1000)) {
            cheater.doCherryAutoban("FASTATTACK");
          }
          if ((cheater.getCheatTracker().getOffenses().get(CheatingOffense.ATTACK_FARAWAY_MONSTER) != null) && (cheater.getCheatTracker().getOffenses().get(CheatingOffense.ATTACK_FARAWAY_MONSTER).getCount() > 1000)) {
            cheater.doCherryAutoban("ATTACK_FARAWAY_MONSTER");
          }
          if ((cheater.getCheatTracker().getOffenses().get(CheatingOffense.ITEMVAC) != null) && (cheater.getCheatTracker().getOffenses().get(CheatingOffense.ITEMVAC).getCount() > 1000)) {
