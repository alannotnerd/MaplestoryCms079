...
public class LoginWorker
  ...
  public void updateLoad() {
    try {
      ...
      for (Entry<Integer, Integer> entry : load.entrySet()) {
        load.put(entry.getKey(), Integer.valueOf(Math.min(1200, (int) (entry.getValue().intValue() * loadFactor))));
      }
  ...
  public void run() {
    try {
      ...
      if ((possibleLogins == 0) && (this.waiting.peek().isGM())) {
        possibleLogins = 1;
      }
      for (int i = 0; i < possibleLogins; i++) {
        ...
        synchronized (this.waiting) {
          ...
          client = this.waiting.removeFirst();
        }
