  ...
  public void messageReceived(IoSession session, Object message) throws Exception {
    ...
    if (log.isTraceEnabled() || log.isInfoEnabled()) {
      ...
      if (packetHandler == null) {
        log.info("发现玩家  {} ({}) {}\n{}", from, content.length, HexTool.toString(content), HexTool.toStringFromAscii(content));
      } else if (log.isTraceEnabled()) {
        log.trace("有消息 {}由 {} ({}) {}\n{}", from, packetHandler.getClass().getSimpleName(), content.length, HexTool.toString(content), HexTool.toStringFromAscii(content));
      }
    }
...
//          System.out.println(String.format("Got Message %s handled by %s (%d) %s\n%s", from, packetHandler.getClass().getSimpleName(), content.length, HexTool.toString(content), HexTool.toStringFromAscii(content)));
        }
