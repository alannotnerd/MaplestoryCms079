...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		if (status == 0) {
			...
			cm.sendNextPrev("Today we're holding a #eFitness Jump Quest Event!#k");
		} else if (status == 2) {
			cm.sendNextPrev("There are a few rules before we start:\r\n#r-You must be level 30\r\n-If you die, you won't be revived\r\n-You must hug the left wall until further notice#n");
		} else if (status == 3) {
			cm.sendNextPrev("#e#rIgnoring the rules may result in a warning or ban!#k#n");
		} else if (status == 4) {
			if (cm.getLevel() >= 30) {
				cm.sendSimple("Would you like to be warped to the Fitness Jump Quest?\r\n#L0#Yes!#l\r\n#L1#No!#l");
			}
			else {
