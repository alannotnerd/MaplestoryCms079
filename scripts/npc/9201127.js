...
if (selection == 0) {
	if(cm.getzb() >= 5000) {
            cm.setzb(-5000);
cm.serverNotice("『绯红骑士团公告』：：【"+ cm.getChar().getName() +"】带领他的队友开始挑战绯红骑士团【提莫】！");  
        cm.summonMob(9500344, 10000000, 200000, 1);提莫 
        }else{
        cm.sendOk("抱歉你没有5000元宝。我不能为您召唤"); 
	cm.dispose();}
} else if (selection == 1) {
	if(cm.haveItem(4021010)) {
cm.serverNotice("『绯红骑士团公告』：【"+ cm.getChar().getName() +"】带领他的队友成功的击败绯红骑士团【提莫】开始挑战第三层");  
        cm.warp(803000800, 0);
	cm.gainItem(4021010, -100);
	cm.dispose();
        }else{ 
