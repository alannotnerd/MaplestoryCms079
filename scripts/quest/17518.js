...
function start(mode, type, selection) {
	...
	if (status == 0) {
      ...
      qm.sendNextPrev("...Hilla?",16);
    } else if (status == 2) {	 
	  qm.sendNextPrev("I told you to go away, but you wouldn't listen.");
    } else if (status == 3) {		
	  qm.sendNextPrev("YOU're the one behind all the nonsense!",16);
	} else if (status == 4) {	
	  qm.sendNextPrev("What of it?");
	} else if (status == 5) {	
	  qm.sendNextPrev("What are you scheming?!",16);
	} else if (status == 6) {	
	  qm.sendNextPrev("Scheming? I'm not scheming. I'm working on getting what I want.");
	} else if (status == 7) {
	  qm.sendNextPrev("I'll stop whatever horrible ritual you've got cooked up!",16);
	} else if (status == 8) {
