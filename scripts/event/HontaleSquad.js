...
function headOne(eim, player) {
	...
	while (iter.hasNext()) {
		...
		if (eim.getPlayerCount() > 0) {
			var pIter = eim.getPlayers().iterator();
			while (pIter.hasNext()) {
				pIter.next().getClient().getSession().write(net.sf.cherry.tools.MaplePacketCreator.serverNotice(6,"The Enormous Creature is Approaching from the Deep Cave..."));
			}
		}
		var theMap = eim.getMapFactory().getMap(240060000);
		var mob = net.sf.cherry.server.life.MapleLifeFactory.getMonster(8810024);
...
function headTwo(eim, player) {
	...
	while (iter.hasNext()) {
		...
		if (eim.getPlayerCount() > 0) {
			var pIter = eim.getPlayers().iterator();
			while (pIter.hasNext()) {
				pIter.next().getClient().getSession().write(net.sf.cherry.tools.MaplePacketCreator.serverNotice(6,"Watch out for Horntail... Be prepared for a long fight."));
			}
		}
		var theMap = eim.getMapFactory().getMap(240060100);
		var mob = net.sf.cherry.server.life.MapleLifeFactory.getMonster(8810025);
