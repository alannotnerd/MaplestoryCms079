...
function playerEntry(eim, player) { 
 ...
 player.getClient().getSession().write(net.sf.cherry.tools.MaplePacketCreator.getClock(600));//600为时间限制10分钟。 
 player.getClient().getSession().write(net.sf.cherry.tools.MaplePacketCreator.serverNotice(5, " 如果你想离开，可以跟我讲话。。")); 
...
function startInstance(eim) { 
 if (eim.getPlayerCount() > 0) { 
  var map = eim.getMapFactory().getMap(mapId, false, false); 
   for (var x = 0; x < 1; x++) { 
    var mob = net.sf.cherry.server.life.MapleLifeFactory.getMonster(8820008); 
