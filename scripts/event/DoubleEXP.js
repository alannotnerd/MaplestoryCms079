...
importPackage(net.sf.cherry.channel);
...
function startEvent() {
	changeRates(em.getChannelServer().getExpRate() * 2);
	refreshDates(cal);
	date = year + "-" + month + "-" + day + " 18:00:00.0";
	timeStamp = java.sql.Timestamp.valueOf(date).getTime();
...
function changeRates(rate) {
	...
	em.getChannelServer().broadcastPacket(net.sf.cherry.tools.MaplePacketCreator.serverNotice(0,"每天16:00-18:00两小时双倍活动时间到了！"));
}
